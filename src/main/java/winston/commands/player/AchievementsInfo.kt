...
class AchievementsInfo : ICommand {
                ...
                player.achievements.general.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.general.size)
                            )
                        )
                    }
                }
                ...
                player.achievements.damage.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.damage.size)
                            )
                        )
                    }
                }
                ...
                player.achievements.maps.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.maps.size)
                            )
                        )
                    }
                }
                ...
                player.achievements.special.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.special.size)
                            )
                        )
                    }
                }
                ...
                player.achievements.support.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.support.size)
                            )
                        )
                    }
                }
                ...
                player.achievements.tank.forEachIndexed { index, achievement ->
                    run {
                        pages.add(
                            Page(
                                PageType.EMBED,
                                getAchievementEmbed(player, achievement, ctx, index, player.achievements.tank.size)
                            )
                        )
                    }
                }
    ...
    private fun getAchievementEmbed(
        player: Player,
        general: General,
        ctx: CommandContext,
        pageNum: Int,
