...
public class Boomed {
    ...
    public void handle(SlashCommandEvent slashCommandEvent) {
        ...
        TrackUser trackUser = new TrackUser(user.getName() + "#" + user.getDiscriminator(), user.getEffectiveAvatarUrl());
        ...
        if (!botVoiceState.inVoiceChannel()) {
            ...
            playerManager.loadAndPlay(slashCommandEvent, playlistLink, trackUser, false, true);
        } else {
            playerManager.loadAndPlay(slashCommandEvent, playlistLink, trackUser, true, true);
        }
    }
}
